...
| Query layer | `graph_queries.py` | `get_node`, `neighbors`, `claims_for_paper`, `contradicting_claims`, `why_blocked`, `search`, `detect_job_dependency_cycles`, `derivation_mechanism_for`/`derivation_mechanism_breakdown`, `attributed_positions_for()` (every claim sharing a subject/object shown as its own attributed position — "according to source A..." — instead of collapsed into one resolved value) — structured answers, not text to parse |
| Roadmap | `graph_roadmap.py` | Rolls GAP/METHOD/DOMAIN/BENCHMARK nodes up by how many papers substantiate each one |
| Evaluation | `graph_evals.py` | Quality, not just correctness: query-answer evals, gate-decision audits, extraction precision/recall, a regression set that must never silently start passing again, corpus-level construction-quality checks (claim field completeness, duplicate text, orphaned nodes), and a repair-pattern effectiveness check (thin evidence — one workshop paper — but a concrete, narrow proxy for whether recorded memory actually leaves behind something reusable) |
